...
////////////////
var p,q,r,s,t=0,u=0,v=!1,w=0,x=0,y=!1,z=0;
...
//if the current property is complex, it has a tab to hold it
y&&z++;y||(this.containerRowFluid.innerHTML="")}var C=!1,D=0;if(d(this.property_order,function(a,b){var d=c.editors[b];"object"===d.schema.type||"array"===d.schema.type?D++:C=!0}),C&&D++,"row-fluid"==this.row_container.className)if(this.row_container.childElementCount>D)v?(this.row_container.removeChild(q.container),this.tabs_holder.childNodes[0].removeChild(q.tab)):(this.row_container.removeChild(this.row_container.childNodes[0]),this.tabs_holder.childNodes[0].removeChild(this.tabs_holder.childNodes[0].childNodes[0])),e(this.tabs_holder.childNodes[0].childNodes[0],"click");else if(this.row_container.childElementCount==D)d(this.property_order,function(a,b){var d=c.editors[b],e="object"==d.schema.type||"array"==d.schema.type;if(!d.property_removed){if(0==t||e){if(0==t){if(e)return void t++;"object"!=c.rows[0].schema.type&&"array"!=c.rows[0].schema.type||c.addRow(d,!0)}else{if(1==t)return void t++;if(t>1)return}t++}var f=c.theme.getGridRow();c.containerRowFluid.appendChild(f),d.options.hidden?d.container.style.display="none":c.theme.setGridColumnSize(d.container,12),f.appendChild(d.container)}}),e(this.tabs_holder.childNodes[0].childNodes[0],"click");else{var E=c.theme.getTabContent();if(y)
//Add a panel
"tab-pane active"!=s.container.className?(E.appendChild(s.container),this.row_container.appendChild(E)):this.row_container.appendChild(s.container),c.tabs_holder.childNodes[0].appendChild(s.tab),e(s.tab,"click");else{var F=this.theme.getGridRow(),G=!0;
//check for a raw container and add one if not
if(this.row_container.childNodes.length>0)if("container-fluid"!=this.row_container.childNodes[0].childNodes[0].className){
//Check for additional property just created
if(
//add a container at 0
this.row_container.insertBefore(E,this.row_container.childNodes[0]),this.containerRowFluid=this.theme.getGridContainer(),this.containerRowFluid.appendChild(F),E.appendChild(this.containerRowFluid),s.options.hidden?s.container.style.display="none":this.theme.setGridColumnSize(s.container,12),F.appendChild(s.container),d(this.rows,function(a,b){return b.schema.type&&"object"!==b.schema.type&&"array"!==b.schema.type?void(G=!1):!b.schema.type&&G?void(G=!1):void 0}),s.schema.type)this.addTab(0),c.tabs_holder.childNodes[0].insertBefore(c.rows[0].tab,c.tabs_holder.childNodes[0].childNodes[0]);else{var H=this.rows.length;if(G){for(var I=H;I>0;I--)this.rows[I]=this.rows[I-1];this.rows[0]=s}for(this.tabs_holder.childNodes[0].childElementCount;this.tabs_holder.childNodes[0].hasChildNodes();)this.tabs_holder.childNodes[0].removeChild(this.tabs_holder.childNodes[0].firstChild);for(var J=0;J<this.rows.length;J++)this.addTab(J),this.tabs_holder.childNodes[0].appendChild(this.rows[J].tab)}this.active_tab=null,e(this.tabs_holder.childNodes[0].childNodes[0],"click")}else this.containerRowFluid.appendChild(F),s&&(s.options.hidden?s.container.style.display="none":this.theme.setGridColumnSize(s.container,12),F.appendChild(s.container));else
//add a container at 0
...
// Sort editors by propertyOrder
this.property_order=Object.keys(this.editors),this.property_order=this.property_order.sort(function(b,c){var d=a.editors[b].schema.propertyOrder,e=a.editors[c].schema.propertyOrder;"number"!=typeof d&&(d=1e3),"number"!=typeof e&&(e=1e3);var f="categories"==a.format&&("object"==a.editors[b].schema.type||"array"==a.editors[b].schema.type),g="categories"==a.format&&("object"==a.editors[c].schema.type||"array"==a.editors[c].schema.type);return"boolean"==typeof f&&f&&(d+=1e4),"boolean"==typeof g&&g&&(e+=1e4),d-e})},addTab:function(a){var b=this,c="object"===b.rows[a].schema.type||"array"===b.rows[a].schema.type;b.tabs_holder&&(b.rows[a].tab_text=document.createElement("span"),
//if (idx == 0){
0!=a&&b.rows[a].schema.type||c?b.rows[a].tab_text.textContent=b.rows[a].getHeaderText():b.rows[a].tab_text.textContent=void 0===b.schema.main_title?"Basic":b.schema.main_title,b.rows[a].tab=b.theme.getTab(b.rows[a].tab_text),b.rows[a].tab.addEventListener("click",function(c){b.active_tab=b.rows[a].tab,b.refreshTabs(),c.preventDefault(),c.stopPropagation()}))},addRow:function(a,b){var c=this,d=this.rows.length;0!=d&&"object"!==a.schema.type&&"array"!==a.schema.type||(c.rows[d]=a,c.addTab(d),c.theme.addTab(c.tabs_holder,c.rows[d].tab))},refreshTabs:function(a){var b=this;d(this.rows,function(c,d){"object"===b.rows[c].schema.type||"array"===b.rows[c].schema.type;d.tab&&(a&&(d.tab_text.textContent=d.getHeaderText()),d.tab===b.active_tab?(b.theme.markTabActive(d.tab),"span12"===d.container.className?b.row_container.children[0].style.display="":d.container.style.display=""):(b.theme.markTabInactive(d.tab),
//&& self.row_container.childElementCount >= self.rows.length
"span12"===d.container.className?b.row_container.children[0].style.display="none":d.container.style.display="none"))})},build:function(){var a=this;
// If the object should be rendered as a table row
